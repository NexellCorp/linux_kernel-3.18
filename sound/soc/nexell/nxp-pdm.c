...
#define	PDM_BASEADDR			PHYS_BASE_PDM
...
static int nxp_pdm_check_param(struct nxp_pdm_snd_param *par)
{
	...
	request = clk_get_rate(par->clk);

	pdm->CTRL = readl(base+PDM_CTRL_OFFSET);
	pdm->CTRL1 = readl(base+PDM_CTRL_OFFSET);
...
static int nxp_pdm_set_plat_param(struct nxp_pdm_snd_param *par, void *data)
{
    ...
    par->sample_rate = sample_rate ? sample_rate : DEF_SAMPLE_RATE;
	...
	dma->dma_ch_name = PL08X_DMA_NAME_PDM;
...
static const struct snd_soc_component_driver nxp_pdm_component = {
    .name       = "nxp-pdmc",
};

static int nxp_pdm_probe(struct platform_device *pdev)
{
	...
	ret = snd_soc_register_component(&pdev->dev, &nxp_pdm_component,
			&pdm_dai_driver, 1);
	if (ret) {
...
static int nxp_pdm_remove(struct platform_device *pdev)
{
...
#ifdef CONFIG_OF
static const struct of_device_id nxp_pdm_match[] = {
		{ .compatible = "nexell,nxp-pdm" },
			{},
};
MODULE_DEVICE_TABLE(of, nxp_pdm_match);
#else
#define nxp_pdm_match NULL
#endif
...
static struct platform_driver pdm_driver = {
	...
	.driver = {
		.name 	= "nxp-pdm",
		.owner 	= THIS_MODULE,
		.of_match_table	= of_match_ptr(nxp_pdm_match),
	},
};
