...
static int nxp_i2s_set_plat_param(struct nxp_i2s_snd_param *par, void *data)
{
	...
	of_property_read_u32(pdev->dev.of_node, "master_mode", &par->master_mode);
   	of_property_read_u32(pdev->dev.of_node, "mclk_in", &par->mclk_in);
   	of_property_read_u32(pdev->dev.of_node, "trans_mode", &par->trans_mode);
   	of_property_read_u32(pdev->dev.of_node, "frame_bit", &par->frame_bit);
	if (!par->frame_bit)
    ...
    for (i = 0; 2 > i; i++, dma = &par->capt) {
		...
		switch (id) {
			case 0:
				dma->dma_ch_name = (char*)(i == 0 ? PL08X_DMA_NAME_I2S0_TX : PL08X_DMA_NAME_I2S0_RX);
				break;
			case 1:
				dma->dma_ch_name = (char*)(i == 0 ? PL08X_DMA_NAME_I2S1_TX : PL08X_DMA_NAME_I2S1_RX);
				break;
			case 2:
				dma->dma_ch_name = (char*)(i == 0 ? PL08X_DMA_NAME_I2S2_TX : PL08X_DMA_NAME_I2S2_RX);
				break;
		}
...
static const struct snd_soc_component_driver nxp_i2s_component = {
    .name       = "nxp-i2sc",
};

static int nxp_i2s_probe(struct platform_device *pdev)
{
	...
	if (ret)
 	...
 	ret = snd_soc_register_component(&pdev->dev, &nxp_i2s_component,
	        &i2s_dai_driver, 1);
    if (ret) {
	    printk(KERN_ERR "fail, %s snd_soc_register_component ...\n", pdev->name);
	    goto err_out;
	}
...
static int nxp_i2s_remove(struct platform_device *pdev)
{
...
#ifdef CONFIG_OF
static const struct of_device_id nxp_i2s_match[] = {
		{ .compatible = "nexell,nxp-i2s" },
			{},
};
MODULE_DEVICE_TABLE(of, nxp_i2s_match);
#else
#define nxp_i2s_match NULL
#endif
...
static struct platform_driver i2s_driver = {
	...
	.driver = {
		.name 	= "nxp-i2s",
		.owner 	= THIS_MODULE,
    	.of_match_table = of_match_ptr(nxp_i2s_match),
	},
};
