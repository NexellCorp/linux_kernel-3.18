...
#define	SPDIF_BASEADDR			PHYS_BASE_SPDIF_RX
...
static const struct snd_soc_component_driver nxp_spdif_component = { 
	    .name       = "nxp-spdif-rxc",
};

static int nxp_spdif_probe(struct platform_device *pdev)
{
    ...
    ret = snd_soc_register_component(&pdev->dev, &nxp_spdif_component,
            &spdif_dai_driver, 1); 
    if (ret) {
...
static int nxp_spdif_remove(struct platform_device *pdev)
{
...
#ifdef CONFIG_OF
static const struct of_device_id nxp_spdif_match[] = { 
	        { .compatible = "nexell,nxp-spdif-rx" },
			            {},
};
MODULE_DEVICE_TABLE(of, nxp_spdif_match);
#else
#define nxp_spdif_match NULL
#endif
...
static struct platform_driver spdif_driver = {
	...
	.driver = {
		.name 	= "nxp-spdif-rx",
		.owner 	= THIS_MODULE,
        .of_match_table = of_match_ptr(nxp_spdif_match),
	},
};
