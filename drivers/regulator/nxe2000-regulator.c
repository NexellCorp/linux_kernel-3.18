...
#define NXE2000_REG(_name, _id, _en_reg, _en_bit, _disc_reg, _disc_bit, _vout_reg, \
...
#if 0
	struct regulator_consumer_supply *consumer_supplies = nxe2000_pdata->initdata->consumer_supplies;
...
#ifdef CONFIG_OF
#if 0
static int nxe2000_regulator_dt_parse_dvs_gpio(struct platform_device *pdev,
			...
			struct device_node *pmic_np)
{
	...
	for (i = 0; i < 3; i++) {
		gpio = of_get_named_gpio(pmic_np,
					"nxe2000,pmic-buck125-dvs-gpios", i);
...
static int nxe2000_regulator_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->num_regulators; i++) {
		...
		err = nxe2000_cache_regulator_register(pdev->dev.parent, nxe2000);
		if (err) {
			dev_err(&pdev->dev, "%s() \e[31mError\e[0m: Fail in caching register\n", __func__);
			return err;
		}

		err = nxe2000_regulator_preinit(pdev->dev.parent, nxe2000, tps_pdata);
		if (err) {
		...
		nxe2000_regulators[i].rdev = regulator_register(&nxe2000->desc, &config);
