					...
					struct axp_platform_data *pdata)
{
	...
	rdata = devm_kzalloc(&pdev->dev, sizeof(*rdata)*pdata->num_regl_devs, GFP_KERNEL);
	if (!rdata) {
	...
	{
		regu_initdata = (void*)of_get_regulator_init_data(&pdev->dev, reg_np);
		...
		regu_initdata->consumer_supplies = devm_kzalloc(&pdev->dev, sizeof(struct regulator_consumer_supply), GFP_KERNEL);
		if(of_property_read_string(reg_np, "regulator-name", &regu_initdata->consumer_supplies->supply))
			dev_err(&pdev->dev, "%s() \e[31mError\e[0m : regulator-name \n", __func__);

		regu_initdata->num_consumer_supplies = 1;
		regu_initdata->constraints.valid_ops_mask = REGULATOR_CHANGE_VOLTAGE | REGULATOR_CHANGE_STATUS;
		...
		if(!of_property_read_u32(reg_np, "nx,init_uV", &val))
			...
			dev_err(&pdev->dev, "%s() \e[31mError\e[0m : nx,init_uV \n", __func__);

		if(!of_property_read_u32(reg_np, "nx,init_enable", &val))
			regu_initdata->constraints.state_standby.enabled = (int)val;
		else
			dev_err(&pdev->dev, "%s() \e[31mError\e[0m : nx,init_enable \n", __func__);
#endif
...
static int axp_regulator_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->num_regl_devs; i++) {
		...
		if (tps_pdata == NULL) {
			dev_err(&pdev->dev, "%s() \e[31mError\e[0m: invalid regulator ID specified(%d)\n", __func__, regl_devs->id);
			return -EINVAL;
		}

		if (tps_pdata->desc.id == AXP22_ID_LDO1 || tps_pdata->desc.id == AXP22_ID_LDO2 \
			|| tps_pdata->desc.id == AXP22_ID_LDO3 || tps_pdata->desc.id == AXP22_ID_LDO4 \
			|| tps_pdata->desc.id == AXP22_ID_LDO5 || tps_pdata->desc.id == AXP22_ID_LDO6 \
			|| tps_pdata->desc.id == AXP22_ID_LDO7 || tps_pdata->desc.id == AXP22_ID_LDO8 \
			|| tps_pdata->desc.id == AXP22_ID_LDO9 || tps_pdata->desc.id == AXP22_ID_LDO10 \
			|| tps_pdata->desc.id == AXP22_ID_LDO11 || tps_pdata->desc.id == AXP22_ID_LDO12 \
			|| tps_pdata->desc.id == AXP22_ID_DCDC1 ||tps_pdata->desc.id == AXP22_ID_DCDC4 \
			|| tps_pdata->desc.id == AXP22_ID_DCDC5)
			tps_pdata->desc.ops = &axp22_ops;

		if (tps_pdata->desc.id == AXP22_ID_LDOIO0 || tps_pdata->desc.id == AXP22_ID_LDOIO1 )
			tps_pdata->desc.ops = &axp22_ldoio01_ops;

		if (tps_pdata->desc.id == AXP22_ID_DCDC2 || tps_pdata->desc.id == AXP22_ID_DCDC3 )
			tps_pdata->desc.ops = &axp22_dcdc_ops;

		//tps_pdata->desc.supply_name = regl_devs->reg_node->name;

		/* Register the regulators */
		config.dev = &pdev->dev;
		config.init_data = (struct regulator_init_data *)regl_devs->platform_data;
		config.driver_data = tps_pdata;
		config.of_node = regl_devs->reg_node;

		axp_regulator_info[i].rdev = regulator_register(&tps_pdata->desc, &config);
		if (IS_ERR(axp_regulator_info[i].rdev)) {
			dev_err(&pdev->dev, "%s() \e[31mError\e[0m: regulator register failed\n", __func__);
			ret = PTR_ERR(axp_regulator_info[i].rdev);
			goto err;
		}

#if 0
		if(tps_pdata->desc.id == AXP22_ID_DCDC1 ||tps_pdata->desc.id == AXP22_ID_DCDC2 \
			|| tps_pdata->desc.id == AXP22_ID_DCDC3 ||tps_pdata->desc.id == AXP22_ID_DCDC4 \
			|| tps_pdata->desc.id == AXP22_ID_DCDC5){
...
#ifdef ENABLE_DEBUG
		printk(KERN_ERR  "\e[31m DTS data \e[0m: %12s, %16s, desc.id:%2d, id:%2d, minV:%7d, maxV:%7d, initV:%7d, enabled:%d\n", 
						config.init_data->constraints.name, config.init_data->consumer_supplies->supply, 
						tps_pdata->desc.id, regl_devs->id, 
						config.init_data->constraints.min_uV, config.init_data->constraints.max_uV,
						config.init_data->constraints.state_standby.uV, config.init_data->constraints.state_standby.enabled);
#endif
		regl_devs++;
	}
	return 0;
...
err:
	while (--i >= 0)
		regulator_unregister(axp_regulator_info[i].rdev);
