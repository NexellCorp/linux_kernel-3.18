...
#if 1//defined(CONFIG_USB_DWCOTG)
...
{
#if 0//!defined(CONFIG_USB_DWCOTG)
	int var;
	uint8_t tmp,val;
#endif
	struct axp_charger *charger;
...
#ifdef CONFIG_OF
static int axp_battery_dt_parse_pdata(struct platform_device *pdev,
					struct axp_platform_data *pdata)
{
	struct axp_mfd_chip *iodev = dev_get_drvdata(pdev->dev.parent);
	struct axp_funcdev_info *rdata;
	struct axp_supply_init_data *sply_initdata;
	struct power_supply_info *battery_data;
	struct device_node *pmic_np, *supplyer_np;
	u32 val;
	...
	supplyer_np = of_find_node_by_name(pmic_np, "supplyer");
	if (!supplyer_np) {
		dev_err(&pdev->dev, "could not find supplyer sub-node\n");
		return -EINVAL;
	}

	pdata->num_sply_devs = 1;

	rdata = devm_kzalloc(&pdev->dev, sizeof(*rdata)*pdata->num_sply_devs, GFP_KERNEL);
	if (!rdata) {
		of_node_put(supplyer_np);
		dev_err(&pdev->dev, "could not allocate memory for axp_funcdev_info data\n");
		return -ENOMEM;
	}

	rdata->reg_node = supplyer_np;
	rdata->name = "axp228-supplyer";

	if(!of_property_read_u32(supplyer_np, "nx,id", &val))
		rdata->id = val;
	else
		dev_err(&pdev->dev, "%s() \e[31mError\e[0m : id \n", __func__);

	battery_data = devm_kzalloc(&pdev->dev, sizeof(struct power_supply_info), GFP_KERNEL);
	if (!battery_data) {
		of_node_put(supplyer_np);
		dev_err(&pdev->dev, "could not allocate memory for power_supply_info data\n");
		return -ENOMEM;
	}
	battery_data->name 				="PTI PL336078";
	battery_data->technology 		= POWER_SUPPLY_TECHNOLOGY_LION;
	battery_data->voltage_max_design = CHGVOL;
	battery_data->voltage_min_design = SHUTDOWNVOL;
	battery_data->use_for_apm 		= 1;

	if(!of_property_read_u32(supplyer_np, "nx,battery_cap", &val))
		battery_data->energy_full_design = (int)val;
	else
		dev_err(&pdev->dev, "%s() \e[31mError\e[0m : id \n", __func__);

	sply_initdata = devm_kzalloc(&pdev->dev, sizeof(struct axp_supply_init_data), GFP_KERNEL);
	if (!sply_initdata) {
		of_node_put(supplyer_np);
		dev_err(&pdev->dev, "could not allocate memory for axp_supply_init_data data\n");
		return -ENOMEM;
	}

	sply_initdata->chgcur 		= STACHGCUR;
	sply_initdata->chgvol 		= CHGVOL;
	sply_initdata->chgend 		= ENDCHGRATE;
	sply_initdata->chgen 		= CHGEN;
	sply_initdata->sample_time 	= ADCFREQ;
	sply_initdata->chgpretime 	= CHGPRETIME;
	sply_initdata->chgcsttime 	= CHGCSTTIME;

	sply_initdata->battery_info = battery_data;
	rdata->platform_data = (void*)sply_initdata;
	pdata->sply_devs = rdata;
...
{
	struct axp_mfd_chip *iodev = dev_get_drvdata(pdev->dev.parent);
	struct axp_platform_data *axp_pdata = iodev->pdata;
	...
	{
		err = axp_battery_dt_parse_pdata(pdev, axp_pdata);
		if (err)
		{
	...
	pdata = (struct axp_supply_init_data *)axp_pdata->sply_devs->platform_data;
