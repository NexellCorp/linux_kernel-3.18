...
static int nxp_fb_alloc_mem(struct fb_info *info)
{
	...
	if (dev->fb_phy_base) {
		...
		dev->fb_vir_base = dma_alloc_coherent(device, dev->fb_phy_len,
								(dma_addr_t*)&dev->fb_phy_base, GFP_KERNEL);
		dev->fb_pan_phys = dev->fb_phy_base;
		dev->fb_remapped = 0;
	}
#endif
...
static void nxp_fb_free_mem(struct fb_info *info)
{
	...
	if(dev->fb_vir_base) {
		...
		if (dev->fb_remapped) {
			...
			struct device *device = par->info->device;
			dma_free_coherent(device, dev->fb_phy_len, dev->fb_vir_base, dev->fb_phy_base);
		}
#endif
		dev->fb_vir_base = 0;
		dev->fb_remapped  = 0;
...
static int nxp_fb_get_dt(struct platform_device *pdev)
{
	...
	match = of_match_node(fb_dt_match, pdev->dev.of_node);
	RET_ASSERT_VAL(match, -EINVAL);

	ret = of_property_read_u32(pdev->dev.of_node, "module", &module);
	RET_ASSERT_VAL(0 == ret || 0 == module || 1 == module, -EINVAL);

	plat = (struct nxp_fb_plat_data *)match->data;
	pdev->id = module;

	of_property_read_u32(np, "frame_layer", &plat->layer);
	of_property_read_u32(np, "frame_format", &plat->format);
	of_property_read_u32(np, "back_color", &plat->bgcolor);
	of_property_read_u32(np, "bit_per_pixel", &plat->bitperpixel);
	of_property_read_u32(np, "lcd_x_resol_max", &plat->x_resol_max);
	of_property_read_u32(np, "lcd_y_resol_max", &plat->y_resol_max);
	of_property_read_u32(np, "lcd_x_resol", &plat->x_resol);
	of_property_read_u32(np, "lcd_y_resol", &plat->y_resol);
	of_property_read_u32(np, "frame_buffers", &plat->buffers);
...
#ifdef CONFIG_OF
struct nxp_fb_plat_data fb_dt_data = {
};

static const struct of_device_id fb_dt_match[] = {
	{ .compatible = "nexell,nxp-fb", .data = (void*)&fb_dt_data, },
	{},
};
MODULE_DEVICE_TABLE(of, fb_dt_match);
#else
#define fb_dt_match NULL
#endif

static struct platform_driver fb_plat_driver = {
	.probe		= nxp_fb_probe,
	.remove		= nxp_fb_remove,
	.suspend	= nxp_fb_suspend,
	.resume		= nxp_fb_resume,
	.driver		= {
		.name	= DEV_NAME_FB,
		.owner	= THIS_MODULE,
		.of_match_table	= of_match_ptr(fb_dt_match),
	},
};
