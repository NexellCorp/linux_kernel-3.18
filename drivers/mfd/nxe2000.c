...
void nxe2000_power_off(void)
{
	...
	ret = nxe2000_read(&nxe2000_i2c_client->dev, NXE2000_REG_TIMSET, &reg_val);
	if (ret < 0)
		ret = nxe2000_read(&nxe2000_i2c_client->dev, NXE2000_REG_TIMSET, &reg_val);

	reg_val |= 0x03;

	ret = nxe2000_write(&nxe2000_i2c_client->dev, NXE2000_REG_TIMSET, reg_val);
	if (ret < 0)
		ret = nxe2000_write(&nxe2000_i2c_client->dev, NXE2000_REG_TIMSET, reg_val);
	if (ret < 0)
		dev_err(&nxe2000_i2c_client->dev, "Error in writing the NXE2000_REG_TIMSET\n");
...
void nxe2000_restart(char str, const char *cmd)
{
	...
	/* Not repeat power ON after power off(Power Off/N_OE) */
	reg_val = (0x1<<1) | 1;
	ret = nxe2000_write(&nxe2000_i2c_client->dev, NXE2000_REG_REPCNT, reg_val);
	if (ret < 0)
		ret = nxe2000_write(&nxe2000_i2c_client->dev, NXE2000_REG_REPCNT, reg_val);
...
static int nxe2000_gpio_get(struct gpio_chip *gc, unsigned offset)
{
	...
	ret = nxe2000_read(nxe2000->dev, NXE2000_REG_MON_IOIN, &val);
	if (ret < 0)
		return ret;
			...
			int value)
{
	struct nxe2000 *nxe2000 = container_of(gc, struct nxe2000,
								 gpio_chip);
	if (value)
		nxe2000_set_bits(nxe2000->dev, NXE2000_REG_IOOUT, 1 << offset);
	else
		nxe2000_clr_bits(nxe2000->dev, NXE2000_REG_IOOUT, 1 << offset);
}
...
static int nxe2000_gpio_input(struct gpio_chip *gc, unsigned offset)
{
	...
	return nxe2000_clr_bits(nxe2000->dev, NXE2000_REG_IOSEL, 1 << offset);
}
				...
				int value)
{
	...
	return nxe2000_set_bits(nxe2000->dev, NXE2000_REG_IOSEL, 1 << offset);
}
	...
	struct nxe2000_platform_data *pdata)
{
	...
	for (i = 0; i < pdata->num_gpioinit_data; ++i) {
		...
		if (ginit->output_mode_en) {
			...
			if (ginit->output_val)
				/* output H */
				ret = nxe2000_set_bits(nxe2000->dev, NXE2000_REG_IOOUT, 1 << i);
			else
				/* output L */
				ret = nxe2000_clr_bits(nxe2000->dev, NXE2000_REG_IOOUT, 1 << i);
			if (!ret)
				ret = nxe2000_set_bits(nxe2000->dev, NXE2000_REG_IOSEL, 1 << i);
		} else
			/* GPIO input mode */
			ret = nxe2000_clr_bits(nxe2000->dev, NXE2000_REG_IOSEL, 1 << i);

		/* if LED function enabled in OTP */
		if (ginit->led_mode) {
			/* LED Mode 1 */
			if (i == 0)	/* GP0 */
				ret = nxe2000_set_bits(nxe2000->dev, NXE2000_REG_GPLED_FUNC, 0x04 | (ginit->led_func & 0x03));
			if (i == 1)	/* GP1 */
				ret = nxe2000_set_bits(nxe2000->dev, NXE2000_REG_GPLED_FUNC, 0x40 | (ginit->led_func & 0x03) << 4);
		}
...
static void nxe2000_noe_init(struct nxe2000 *nxe2000)
{
	...
	__nxe2000_write(client, NXE2000_REG_REPCNT, 0x1);
#endif
}
...
#ifdef CONFIG_OF
static struct of_device_id nxe2000_pmic_dt_match[] = {
	{.compatible = "nx,nxe2000", .data = NULL},
	{},
};

static struct nxe2000_platform_data *nxe2000_i2c_parse_dt_pdata(struct device *dev)
{
			      ...
			      const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		match = of_match_device(of_match_ptr(nxe2000_pmic_dt_match), &client->dev);
		if (!match) {
	...
	if (nxe2000->dev->of_node) {
		pdata = nxe2000_i2c_parse_dt_pdata(nxe2000->dev);
		if (IS_ERR(pdata)) {
			ret = PTR_ERR(pdata);
...
//	ret = pdata->init_port(client->irq);
	...
	ret = mfd_add_devices(nxe2000->dev, -1, nxe2000_devs, ARRAY_SIZE(nxe2000_devs), NULL, 0, NULL);
