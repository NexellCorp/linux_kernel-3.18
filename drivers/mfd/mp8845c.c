...
#ifdef CONFIG_OF
static struct of_device_id mp8845c_pmic_dt_match[] = {
	{.compatible = "nx,mp8845c", .data = NULL},
	{},
};

static struct mp8845c_platform_data *mp8845c_i2c_parse_dt_pdata(struct device *dev)
{
			      ...
			      const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		match = of_match_device(of_match_ptr(mp8845c_pmic_dt_match), &client->dev);
		if (!match) {
	...
	ret = mfd_add_devices(mp8845c->dev, -1, &mp8845c_devs[pdata->id], 1, NULL, 0, NULL);
...
#ifdef CONFIG_PM
	.suspend = mp8845c_i2c_suspend,
	.resume = mp8845c_i2c_resume,
#endif
