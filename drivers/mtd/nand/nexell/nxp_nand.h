 ...
 * MA 02111-1307 USA
 */
#ifndef __NXP_NAND_H__
#define __NXP_NAND_H__
...
//xx #define	ECC_PAGE_SIZE	CFG_NAND_ECC_BYTES
//xx #define	ECC_HW_BITS		CFG_NAND_ECC_BITS
//xx #define	ECC_BCH_BITS	CFG_NAND_ECC_BITS
...
struct nxp_nand_cfg {
	int chip_delay;					/* unit: us */
	int wp_gpio;

	uint8_t tacs;					/* unit: bclk cycle */
	uint8_t tcah;
	uint8_t tcos;
	uint8_t tcoh;
	uint8_t tacc;

	uint32_t ecc_unit_size;
	uint32_t ecc_hw_bits;
	uint32_t ecc_bch_bits;
...
struct nxp_nand {
	...
	void __iomem *io_ctrl;			// CONTROL  : NFCONTROL, NFECCTRL, ...
	void __iomem *io_intf;			// INTERFACE: NFDATA, NFCMD, NFADDR
	struct clk *clk;
	uint32_t clkhz;
