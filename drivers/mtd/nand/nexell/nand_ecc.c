 ...
 * u-boot nand hw ecc
 */
static int iNX_BCH_VAR_K	 = 1024;	/* 512 or 1024 */
static int iNX_BCH_VAR_M	 = 14;		/* 13, 14 */
...
static int __ecc_get_err_location(unsigned int *pLocation, uint32_t ecc_hw_bits)
{
	volatile U32 *pELoc = _pNCTRL->NFERRLOCATION;
	volatile int len = (int)(ecc_hw_bits/2);
	volatile int err, i;
				...
				uint8_t *buf, int oob_required, int page)
{
	...
	DBGOUT("%s, page=%d, ecc mode=%u, bytes=%d, page size=%d, step=%d\n",
	...
	do {
		...
		for (n = 0; eccsteps; eccsteps--, p += eccsize) {
			...
			if (err) {
				...
				errcnt = __ecc_get_err_location((unsigned int *)errpos, ecc_hw_bits);
...
int nand_ecc_layout_hwecc(struct mtd_info *mtd)
{
	...
	if (ecctotal > oobsize)  {
		printk(KERN_INFO "\n");
		printk(KERN_INFO "==================================================\n");
		printk(KERN_INFO "error: %u bit hw ecc mode requires ecc %d byte	\n", ecc_hw_bits, ecctotal);
...
int nand_hw_ecc_init_device(struct mtd_info *mtd)
{
	...
	DBGOUT("%s ecc %u bit, eccsize=%d, parity=%d, eccbyte=%d, eccindex=%d\n",
	...
	errpos = kmalloc(sizeof(int) * ecc_hw_bits, GFP_KERNEL);
...
_ecc_fail:
	printk("Fail: not support ecc %u bits for pagesize %d !!!\n", ecc_hw_bits, eccsize);
	return -EINVAL;
}
