...
static int _configure(struct nxp_vin_clipper *me, int enable)
{
    ...
    if (enable) {
...
#else
            me->platdata->power_enable(me->platdata, true);
#endif
        if (me->platdata->set_clock)
#ifndef CONFIG_OF
            me->platdata->set_clock(me->platdata->clk_rate);
#else
            me->platdata->set_clock(me->platdata, enable);
#endif
        ...
        if (me->platdata->power_state_changed) {
            ...
            if (me->platdata->power_state_changed())
#else
            if (me->platdata->power_state_changed(me->platdata))
#endif
        ...
        if (me->platdata->late_power_down)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,18,0)
            queue_delayed_work(system_highpri_wq, &me->work_power_down, msecs_to_jiffies(LATE_POWERDOWN_TIMEOUT_MS));
#else
...
#else
            me->platdata->setup_io(me->platdata, false);
#endif
...
#else
        me->platdata->setup_io(me->platdata, true);
#endif
    _hw_set_clock(me, true);
