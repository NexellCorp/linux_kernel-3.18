...
static inline int _init_dma(u8 ch_num, struct ts_drv_context *ctx)
{
	...
	case NXP_MP2TS_ID_CAP0:
		ctx->ch_info[ch_num].filter_data	= PL08X_DMA_NAME_MPEGTSI0;
		break;
	case NXP_MP2TS_ID_CAP1:
		ctx->ch_info[ch_num].filter_data	= PL08X_DMA_NAME_MPEGTSI1;
		break;
	case NXP_MP2TS_ID_CORE:
		ctx->ch_info[ch_num].filter_data	= PL08X_DMA_NAME_MPEGTSI2;
		break;
	case NXP_MP2TS_ID_MAX:
		ctx->ch_info[ch_num].filter_data	= PL08X_DMA_NAME_MPEGTSI3;
		break;
	}
...
static u64 nxp_mp2ts_dmamask = DMA_BIT_MASK(32);
...
static inline int _init_buf(struct ts_drv_context *ctx, struct ts_buf_init_info *init_info)
{
	...
	ctx->dev->dma_mask = &nxp_mp2ts_dmamask;	
...
onemore_alloc:
    ...
    ctx->ch_info[ch_num].alloc_size     = PAGE_ALIGN(ctx->ch_info[ch_num].alloc_align * init_info->page_num);

 	ctx->ch_info[ch_num].dma_virt = dma_alloc_writecombine(ctx->dev, ctx->ch_info[ch_num].alloc_size, &ctx->ch_info[ch_num].dma_phy, GFP_ATOMIC);
...
static int _init_context(struct ts_drv_context *ctx, struct nxp_mp2ts_plat_data *pdata)
{
    ...
    ctx->baseaddr = (void __iomem *)IO_ADDRESS(PHYS_BASE_MPEGTSI);
...
static int _init_device(struct ts_drv_context *ctx)
{
    ...
    u32 addr;
	struct reset_control *rst = devm_reset_control_get(ctx->dev,"mp2ts-reset");
    MP2TS_DBG(" %s ++\n", __func__);

    // Clock control
    mp2ts_clk = clk_get(NULL,"mpeg-tsi");
    clk_prepare_enable(mp2ts_clk);
...
static ssize_t mpegts_read_buf(u8 ch_num, struct ts_param_descr *param_descr)
{
	...
	{
		s_ctx->condition = 0;
		s_ctx->wait_ch = ch_num;
		if (wait_time)
		{
			//ret = interruptible_sleep_on_timeout(&s_ctx->ch_info[ch_num].wait, wait_time);
			ret = wait_event_interruptible_timeout(s_ctx->ch_info[ch_num].wait, s_ctx->condition ,wait_time);
			if (ret == 0)
			{
				MP2TS_DBG("time out \n");
			}
	...
	src_addr = (U32)(s_ctx->ch_info[ch_num].dma_virt + (s_ctx->ch_info[ch_num].alloc_align * s_ctx->ch_info[ch_num].r_pos));
...
static ssize_t mpegts_write_buf(u8 ch_num, struct ts_param_descr *param_descr)
{
	...
	dst_addr = (U32)(s_ctx->ch_info[ch_num].dma_virt + (s_ctx->ch_info[ch_num].alloc_align * s_ctx->ch_info[ch_num].w_pos));
...
static int _prepare_dma_submit(u8 ch_num, struct ts_drv_context *ctx)
{
    ...
    if (ctx->ch_info[ch_num].tx_mode == 1)
	{
		...
		slave_config.dst_addr 		= (PHYS_BASE_MPEGTSI + 0x10) + (ch_num * 0x04);
    ...
    else
	{
		...
		slave_config.src_addr 		= (PHYS_BASE_MPEGTSI + 0x10) + (ch_num * 0x04);
		slave_config.src_addr_width = DMA_SLAVE_BUSWIDTH_4_BYTES;
...
static struct platform_device_id nxp_mp2ts_ids[] = {
	{
	        .name       = "nxp-mp2ts",
	}, { },
};
MODULE_DEVICE_TABLE(platform, nxp_mp2ts_ids);


#ifdef CONFIG_OF
static const struct of_device_id  nxp_mp2tsc_match[] = {
     { .compatible = "nexell,nxp-mp2ts" },
     {},
};
MODULE_DEVICE_TABLE(of, nxp_mp2tsc_match);
#else
#define s3c24xx_i2c_match NULL
#endif

static void nxp_mp2ts_parse_dt(struct device_node *np, struct nxp_mp2ts_plat_data *plat  )
{
	u32 key;
	...
	if(!np)
		return;
	of_property_for_each_u32(np, "nxp_dma_size", prop,p,key) {
		plat->ts_dma_size[i++] = key * TS_PAGE_SIZE;	
	}

	i = 0;
	of_property_for_each_u32(np, "nxp_mp2ts_demod_rst" , prop,p,key) {
		plat->dev_info[i++].demod_rst_num = key ;	
	}
	i = 0;
	of_property_for_each_u32(np, "nxp_mp2ts_demod_irq" , prop,p,key) {
		plat->dev_info[i++].demod_irq_num = key ;	
	}
	i = 0;
	of_property_for_each_u32(np, "nxp_mp2ts_tuner_rst" , prop,p,key) {
		plat->dev_info[i++].tuner_rst_num = key ;	
	}
}
...
static int nexell_mpegts_probe(struct platform_device *pdev)
{
    ...
    struct nxp_mp2ts_plat_data *mp2ts_plat = NULL;// pdev->dev.platform_data;
    struct ts_buf_init_info     buf_info;
	...
	if (!pdev->dev.of_node) {
		mp2ts_plat = pdev->dev.platform_data;
		if(!mp2ts_plat) {
			dev_err(&pdev->dev, "no platform data\n");
			return -EINVAL;
		}
	} else {
		mp2ts_plat = devm_kzalloc(&pdev->dev, sizeof(struct  nxp_mp2ts_plat_data), GFP_KERNEL );
		nxp_mp2ts_parse_dt(pdev->dev.of_node, mp2ts_plat);
	}

	//rst = devm_reset_control_get(pdev->dev,"mp2ts-reset");
    ...
    if (ret) {
		...
		MP2TS_DBG("Failed to request irq, err: %d\n", ret);
        goto fail_irq;
    }
#endif
...
static struct platform_driver nexell_mpegts_driver = {
	...
	.id_table	= nxp_mp2ts_ids,
    .driver     = {
        .owner  = THIS_MODULE,
        .name   = DEV_NAME_MPEGTSI,
		.of_match_table = nxp_mp2tsc_match,
    },
};
