...
static struct pin_config {
	...
	{ "nexell,pin-strength", PINCFG_TYPE_STRENGTH },
};
					...
					unsigned group, bool enable)
{
	...
	io = grp->pins[0];
	nxp_soc_gpio_set_io_func(io, func->val);
}
				...
				unsigned long *config, bool set)
{
	...
	if (set) {
		...
		case PINCFG_TYPE_STRENGTH:
			nxp_soc_gpio_set_io_drv(io, cfg_value);
			break;
		default:
			break;
		}
		...
		switch (cfg_type) {
		case PINCFG_TYPE_PULL:
			data = nxp_soc_gpio_get_io_pull_sel(io);
			break;
		case PINCFG_TYPE_STRENGTH:
			data = nxp_soc_gpio_get_io_drv(io);
			break;
		default:
			break;
		}
...
static void nexell_gpio_set(struct gpio_chip *gc, unsigned offset, int value)
{
	struct nexell_pin_bank *bank = gc_to_pin_bank(gc);
	struct nexell_pin_bank_type *type = bank->type;
	unsigned long flags;
	void __iomem *reg;
	u32 data;
	int io;
...
static int nexell_gpio_get(struct gpio_chip *gc, unsigned offset)
{
	void __iomem *reg;
	u32 data;
	struct nexell_pin_bank *bank = gc_to_pin_bank(gc);
	...
	io = bank->grange.pin_base + offset;
	data = nxp_soc_gpio_get_in_value(io);
 ...
 */
static int nexell_gpio_set_direction(struct gpio_chip *gc,
					     unsigned offset, bool input)
{
	...
	io = bank->grange.pin_base + offset;
	fn = GET_GPIO_ALTFUNC(bank->grange.id, offset);

	//nxp_soc_gpio_set_io_func(io, fn);
	nxp_soc_gpio_set_io_dir(io, input ? 0 : 1);

	return 0;
}

/* gpiolib gpio_direction_input callback function. */
static int nexell_gpio_direction_input(struct gpio_chip *gc, unsigned offset)
{
	return nexell_gpio_set_direction(gc, offset, true);
}

/* gpiolib gpio_direction_output callback function. */
static int nexell_gpio_direction_output(struct gpio_chip *gc, unsigned offset,
							int value)
{
	nexell_gpio_set(gc, offset, value);
	return nexell_gpio_set_direction(gc, offset, false);
}
 ...
 */
static int nexell_gpio_to_irq(struct gpio_chip *gc, unsigned offset)
{
#if 0
...
static struct nexell_pin_group *nexell_pinctrl_create_groups(
				struct device *dev,
				struct nexell_pinctrl_drv_data *drvdata,
				unsigned int *cnt)
{
				...
				struct nexell_pmx_func *func)
{
	...
	for (i = 0; i < npins; ++i) {
		...
		ret = of_property_read_string_index(func_np, "nexell,pins",
							i, &gname);
		if (ret) {
			dev_err(dev,
...
static struct nexell_pmx_func *nexell_pinctrl_create_functions(
				struct device *dev,
				struct nexell_pinctrl_drv_data *drvdata,
				unsigned int *cnt)
{
				    ...
				    struct nexell_pinctrl_drv_data *drvdata)
{
	...
	//pr_debug("ptr: %p, %p, ctr: %p, %p, nr_banks: %d\n",
	//		pdev, drvdata, ctrl, bank, ctrl->nr_banks);
...
static struct nexell_pin_ctrl *nexell_pinctrl_get_soc_data(
				struct nexell_pinctrl_drv_data *d,
...
static const struct of_device_id nexell_pinctrl_dt_match[] = {
	{ .compatible = "nexell,s5p6818-pinctrl", .data = (void *)s5p6818_pin_ctrl },
	{},
};
...
static int __init nexell_pinctrl_drv_register(void)
{
	 ...
	 * ops that turn off pad retention (like soc_pm_resume).
	 */
