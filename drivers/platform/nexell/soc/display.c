...
#define	DEV_NAME_DISP	"nxp-display"
...
static inline void set_syncgen_param(struct disp_sync_par *src, struct disp_sync_par *dst)
{
	if (!src || !dst)
		return;
...
static int  disp_syncgen_prepare(struct disp_control_info *info)
{
	...
	if (pspar->vs_start_offset != 0 ||
		...
		pspar->ev_start_offset != 0 ||
		pspar->ev_end_offset   != 0) {
	...
	NX_DPC_SetMode(module, out_format, interlace, invert_field, RGBMode,
			swap_RB, yc_order, EmbSync, EmbSync,
	...
	pr_debug("%s: display.%d (y=%4d, vfp=%3d, vbp=%3d, vsw=%3d)\n",
		...
		psync->v_back_porch, psync->v_sync_width);
	pr_debug("%s: display.%d clk 0[s=%d, d=%3d], 1[s=%d, d=%3d], inv[%d:%d]\n",
		__func__, module, clk_src_lv0, clk_div_lv0, clk_src_lv1, clk_div_lv1,
		pspar->clk_inv_lv0, pspar->clk_inv_lv1);
				...
				unsigned int color, int enable)
{
	...
	case RGB_COLOR_TRANSP:
		...
		NX_MLC_SetTransparency(module, layer,
			(enable ? CTRUE : CFALSE), (color & 0x00FFFFFF));
		NX_MLC_SetDirtyFlag(module, layer);
		break;
 ...
 * RGB Layer on MultiLayer
 */
int nxp_soc_disp_video_set_format(int module, unsigned int fourcc,
...
void nxp_soc_disp_video_set_address(int module,
							...
							int waitvsync)
{
	...
	pr_debug("%s: %s, lua=0x%x, cba=0x%x, cra=0x%x (lus=%d, cbs=%d, crs=%d), wait=%d\n",
...
void 	nxp_soc_disp_video_get_address(int module,
						unsigned int *lu_a, unsigned int *lu_s,
						unsigned int *cb_a, unsigned int *cb_s,
						unsigned int *cr_a, unsigned int *cr_s)
{
...
void nxp_soc_disp_video_set_crop(int module, bool enable,
							int left, int top, int width, int height,
							int waitvsync)
{
	DISP_MULTILY_VID(module, pvid);
...
{
	DISP_MULTILY_DEV(module, pmly);
	nxp_soc_disp_rgb_set_color(module,
		pmly->fb_layer, RGB_COLOR_TRANSP, color, enable);
}

unsigned int nxp_soc_disp_video_get_colorkey(int module)
{
	DISP_MULTILY_DEV(module, pmly);
	return nxp_soc_disp_rgb_get_color(module,
...
int	nxp_soc_disp_device_set_dev_param(enum disp_dev_type device, void *param)
{
	...
	pr_debug("%s: %s, param = %p \n", __func__,
		dev_to_str(device), pdev->dev_param);
			...
			struct platform_device *pldev)
{
	...
	info->wait_time = 4;
	info->condition = 0;
	info->irqno = (info->module == 0 ? IRQ_DPC_P : IRQ_DPC_S);
...
static int display_soc_probe(struct platform_device *pldev)
{
	...
	if (pldev->dev.of_node) {
		ret = of_property_read_u32(pldev->dev.of_node, "module", &module);
	...
	pdev->base_addr = (unsigned long)NX_DPC_GetBaseAddress(module);
	...
	pmly->base_addr = (unsigned long)NX_MLC_GetBaseAddress(module);
	pmly->save_addr = (unsigned long)&save_multily[module];
	pmly->mem_lock_len = 16;	/* fix mem lock size, pspar->mem_lock_size */
