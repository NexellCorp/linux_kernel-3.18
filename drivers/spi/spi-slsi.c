                ...
                struct spi_device *spi)
{
...
#endif
	of_property_read_u32(data_np, "spi-feedback-delay", &fb_delay);
    ...
    of_property_read_u32(data_np, "hierarchy", &hierarchy);
    cs->hierarchy = hierarchy;
    of_node_put(data_np);
    cs->set_level = gpio_set_value;
		                        ...
		                        struct platform_device *pdev)
{
    ...
    if (pdev->dev.of_node) {
        ...
        match = of_match_node(nexell_spi_dt_match, pdev->dev.of_node);
...
static int __init s3c64xx_spi_probe(struct platform_device *pdev)
{
	...
	sdd->port_conf = s3c64xx_spi_get_port_config(pdev);
	sci->fifo_lvl_mask = sdd->port_conf->fifo_lvl_mask;
	sci->clk_from_cmu  = sdd->port_conf->clk_from_cmu ;
	sci->tx_st_done    = sdd->port_conf->tx_st_done;
...
static struct s3c64xx_spi_port_config nexell_spi_port_config = {
    .fifo_lvl_mask  =  0x1ff,
    .rx_lvl_offset  = 15,
    .tx_st_done = 25,
    .high_speed = true,
...
static struct platform_device_id nexell_spi_driver_ids[] = {
	{
		.name       = "s3c2443-spi",
	    .driver_data    = (kernel_ulong_t)&nexell_spi_port_config,
	},
	{ },
};
#endif
...
static const struct of_device_id nexell_spi_dt_match[] = {
	{ .compatible = "nexell,nxp-spi",
	  .data = (void *)&nexell_spi_port_config,
    },   
    { }, 
};
MODULE_DEVICE_TABLE(of, nexell_spi_dt_match);
#endif /* CONFIG_OF */
...
static struct platform_driver s3c64xx_spi_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
		.pm = &s3c64xx_spi_pm,
		.of_match_table = of_match_ptr(nexell_spi_dt_match), 
