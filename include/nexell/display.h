...
struct disp_multily_dev {
	...
	int	interlace;
	int mem_lock_len;	/* lock size for memory access, 4, 8, 16 only valid (default 8byter) */
	unsigned long base_addr;
	unsigned long save_addr;
...
struct nxp_fb_plat_data {
    ...
    int skip_pan_vsync;
    int force_resolution;	/* set resolutioin with fb data, default get from display device */
};
